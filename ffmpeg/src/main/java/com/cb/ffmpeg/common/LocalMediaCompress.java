...
import com.cb.ffmpeg.model.OnlyCompressOverBean;
...
public class LocalMediaCompress extends MediaRecorderBase {
    ...
    private void startCompress(final FFmpegCallBack callBack) {
        ...
        final boolean b = doCompress(true, callBack);
        ...
        if (callBack != null) {
            AsyncTaskUtil.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (b) {
                        callBack.onSuccess(mOnlyCompressOverBean.getVideoPath());
                    }
                    callBack.onFinish();
                }
            });
