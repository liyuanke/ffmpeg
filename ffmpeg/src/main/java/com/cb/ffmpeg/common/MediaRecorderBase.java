...
//		TODO 设置浏览尺寸
    ...
    public Boolean doCompress(boolean mergeFlag, final FFmpegCallBack callBack) {
        Log.e("HongLi", "doCompress compressConfig:" + compressConfig);
        if (compressConfig != null) {
            ...
            final boolean transcodingFlag = FFmpegBridge.jxFFmpegCMDRun(cmd_transcoding) == 0;
            ...
            if (callBack != null) {
                AsyncTaskUtil.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (!transcodingFlag) {
                            callBack.onError("视频转码失败");
                        } else if (!captureFlag) {
                            callBack.onError("视频获取缩略图失败");
                        }
                    }
                });
            }
    ...
    public void setTranscodingFrameRate(int rate) {
        this.mFrameRateCmd = String.format(" -r %d", rate);
    }


    public String getBitrateModeCommand(BaseMediaBitrateConfig config, String defualtCmd, boolean needSymbol) {
        String add = "";
    ...
    public String getBitrateCrfSize(BaseMediaBitrateConfig config, String defualtCmd, boolean nendSymbol) {
        if (TextUtils.isEmpty(defualtCmd)) {
            defualtCmd = "";
        }
        String add = "";
    ...
    public String getBitrateVelocity(BaseMediaBitrateConfig config, String defualtCmd, boolean nendSymbol) {
        if (TextUtils.isEmpty(defualtCmd)) {
            defualtCmd = "";
        }
        String add = "";
